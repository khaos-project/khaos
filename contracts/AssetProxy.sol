...
import './KhaosERC20.sol';
import './KToken2Interface.sol';
import './AssetInterface.sol';
import './ERC20Interface.sol';
import './AssetProxyInterface.sol';
import '../deps/contracts/Bytes32.sol';
import '../deps/contracts/ReturnData.sol';


/**
 * @title KToken2 Asset Proxy.
 *
 * Proxy implements ERC20 interface and acts as a gateway to a single KToken2 asset.
 * Proxy adds ktoken2Symbol and caller(sender) when forwarding requests to KToken2.
 * Every request that is made by caller first sent to the specific asset implementation
 * contract, which then calls back to be forwarded onto KToken2.
 *
 * Calls flow: Caller ->
 *             Proxy.func(...) ->
...
contract AssetProxy is ERC20Interface, AssetProxyInterface, Bytes32, ReturnData {
    // Assigned KToken2, immutable.
    KToken2Interface public ktoken2;

    // Assigned symbol, immutable.
    bytes32 public ktoken2Symbol;
    ...
    /**
     * Sets KToken2 address, assigns symbol and name.
     *
     * Can be set only once.
     *
     * @param _ktoken2 KToken2 contract address.
     * @param _symbol assigned symbol.
     * @param _name assigned name.
     *
     * @return success.
     */
    function init(KToken2Interface _ktoken2, string memory _symbol, string memory _name)
        public returns(bool)
    {
        if (address(ktoken2) != address(0)) {
            return false;
        }
        ktoken2 = _ktoken2;
        ktoken2Symbol = _bytes32(_symbol);
        name = _name;
        symbol = _symbol;
        return true;
    }
    ...
    /**
     * Only KToken2 is allowed to call.
     */
    modifier onlyKToken2() {
        if (msg.sender == address(ktoken2)) {
            _;
        }
    }
    ...
    modifier onlyAssetOwner() {
        if (ktoken2.isOwner(msg.sender, ktoken2Symbol)) {
            _;
        }
    }
    ...
    function totalSupply() public view returns(uint) {
        return ktoken2.totalSupply(ktoken2Symbol);
    }
    ...
    function balanceOf(address _owner) public view returns(uint) {
        return ktoken2.balanceOf(_owner, ktoken2Symbol);
    }
    ...
    function allowance(address _from, address _spender) public view returns(uint) {
        return ktoken2.allowance(_from, _spender, ktoken2Symbol);
    }
    ...
    function decimals() public view returns(uint8) {
        return ktoken2.baseUnit(ktoken2Symbol);
    }
     ...
     * @param _to holder address to give to.
     * @param _value amount to transfer.
     * @param _reference transfer comment to be included in a KToken2's Transfer event.
     *
     * @return success.
     */
    ...
    /**
     * Performs transfer call on the KToken2 by the name of specified sender.
     *
    ...
    public onlyImplementationFor(_sender) returns(bool) {
        return ktoken2.proxyTransferFromWithReference(
            _from,
            _to,
            _value,
            ktoken2Symbol,
    ...
    /**
     * Performs allowance transfer to ICAP call on the KToken2 by the name of specified sender.
     *
     * Can only be called by asset implementation contract assigned to sender.
     *
     * @param _from holder address to take from.
    ...
    function _forwardTransferFromToICAPWithReference(
        ...
        address _sender)
    public onlyImplementationFor(_sender) returns(bool) {
        return ktoken2.proxyTransferFromToICAPWithReference(
            _from,
            _icap,
            _value,
            _reference,
    ...
    /**
     * Performs allowance setting call on the KToken2 by the name of specified sender.
     *
     * Can only be called by asset implementation contract assigned to sender.
     *
     * @param _spender holder address to set allowance to.
    ...
    public onlyImplementationFor(_sender) returns(bool) {
        return ktoken2.proxyApprove(_spender, _value, ktoken2Symbol, _sender);
    }

    /**
     * Emits ERC20 Transfer event on this contract.
     *
     * Can only be, and, called by assigned KToken2 when asset transfer happens.
     */
    function emitTransfer(address _from, address _to, uint _value) public onlyKToken2() {
     ...
     *
     * Can only be, and, called by assigned KToken2 when asset allowance set happens.
     */
    function emitApprove(address _from, address _spender, uint _value) public onlyKToken2() {
    ...
    function multiAsset() public view returns(KToken2Interface) {
        return ktoken2;
    }
}
