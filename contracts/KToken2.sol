...
contract Kmbi2Enabled {
    ...
    modifier onlyRole(bytes32 _role) {
        if (address(kmbi2) != 0x0 && kmbi2.hasRole(this, _role, msg.sender)) {
            _;
        }
    }

    // Perform only after claiming the node, or claim in the same tx.
    function setupKmbi2(Kmbi2 _kmbi2) returns(bool) {
        if (address(kmbi2) != 0x0) {
            return false;
        }
...
contract Kmbi2EnabledFull is Kmbi2Enabled {
    ...
    function setupKmbi2(Kmbi2 _kmbi2) returns(bool) {
        ...
        if (!_kmbi2.claimFor(this, msg.sender) && !_kmbi2.isOwner(this, msg.sender)) {
            return false;
        }
 ...
 * @title KToken2.
 *
 * The official Kmbisafe assets platform powering all kinds of tokens.
 * KToken2 uses EventsHistory contract to keep events, so that in case it needs to be redeployed
 ...
 * didn't happen yet.
 */
contract KToken2 is Kmbi2EnabledFull {
