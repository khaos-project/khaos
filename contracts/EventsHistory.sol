...
contract KmbiEnabled {
    Kmbi public kmbiC;
    bool public isImmortal;
    bytes32 public name;

    modifier checkAccess(bytes32 _role) {
        if(address(kmbiC) != 0x0 && kmbiC.hasRelation(name, _role, msg.sender)){
            _
        }
    }
    
    function getAddress(bytes32 _name) constant returns (address) {
        return kmbiC.getNodeAddress(_name);
    }

    function setKmbiAddress(address _kmbi, bytes32 _name) returns (bool){
        if(address(kmbiC) != 0x0){
            return false;
        }
        Kmbi kmbiContract = Kmbi(_kmbi);
        if(kmbiContract.getNodeAddress(_name)!=address(this)) {
            if (!kmbiContract.addNode(_name, address(this))){
                return false;
            }
        }
        name = _name;
 ...
 * didn't happen yet.
 */
contract EventsHistory is KmbiEnabled, Safe {
